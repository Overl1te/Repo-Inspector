...
# Repo Inspector — GitHub repository quality scanning via API
...
[Features](#-features) • [Live card examples](#-live-card-examples) • [Quick start](#-quick-start) • [Deploy](#-deploy-pages--vercel) • [API](#-api)
...
- `GET /api/jobs/{job_id}` — scan job status
- `GET /api/report/{job_id}.json` — JSON report
- `GET /api/report/{job_id}.md` — Markdown report
- `GET /api/report/{job_id}.txt` — TXT report
...
```text
Repo-Inspector/
├── app/                    # FastAPI, scanner, templates, localization
├── api/                    # serverless entrypoint for Vercel
├── web/                    # static frontend (Pages)
